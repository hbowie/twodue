...
/**
   A sorted collection of to do items. <p>
  
   This code is copyright (c) 2003 by Herb Bowie.
   All rights reserved. <p>
  
   Version History: <ul><li>
      2003/11/22 - Added ItemNavigator interface. <li>
      2003/11/11 - SortedItems created to perform functions
                   previously performed by ToDoItems. <li>
      2003/09/01 - Originally written as ToDoItems.
       </ul>
   ...
   @version 
      2004/06/07 - Modified to use ToDoColumns and to change list width
                   to 1000.
 */
...
public class SortedItems 
  ...
  public  static final int  LIST_WIDTH_EXTRA = 200;
   ...
   */
	...
	public SortedItems (TwoDueCommon td) {
    this.td = td;
    recDef = ToDoItem.getRecDef();
		items = new ToDoItems(td);
  ...
  public void setDueDateIcons 
    ...
    futureIcon = future;
    columns = new ToDoColumns (late, today, tomorrow, future);
    if (comp != null) {
      columns.setComparator (comp);
    }
    getDisplayColumns();
   ...
   */
  public void setCommon(TwoDueCommon common) {
    this.td = common;
  }
  ...
  public int getPriorityColumn () {
    ...
    for (int i = 0; i < displayColumns.size() && r < 0; i++) {
      ...
      if (column.getColumnNumber() == ToDoItem.PRIORITY) {
        r = i;
      }
    }
    return r;
  ...
  /**
    ...
    @param item The item to be added to the collection.
   */
  private int addInternal (ToDoItem item) {
  ...
  /**
   ...
   @param item  The ToDoItem after any modifications. 
   */
  ...
  /**
    ...
    @param item The item to be added to the collection.
   */
  public int doItem (int operation, ToDoItem item) {
  ...
  /**
    Schedule start times for open items.
   
    @return Number of to do items modified by scheduling operation.
   
    @param defaultStartTime The time at which the first open item on each 
                            day should start. Start times for following items
                            on the same day will be calculated based on
                            successive start times and durations.
   */
  public int scheduleStartTimes (TimeOfDay defaultStartTime) {
    int itemsModified = 0;
    int duration = 0;
    ToDoItem nextItem;
    DateFormat fmt = new SimpleDateFormat ("MM/dd/yyyy");
    String lastDueDate = "";
    TimeOfDay nextStartTime = new TimeOfDay (defaultStartTime);
    for (int itemIndex = 0; itemIndex < size(); itemIndex++) {
      nextItem = get (itemIndex);
      if (! nextItem.isDeleted()
          && nextItem.isNotDone()) {
        String dueDate = nextItem.getDueDate (fmt);
        if (! lastDueDate.equals (dueDate)) {
          nextStartTime.set (defaultStartTime);
          lastDueDate = dueDate;
        }
        if (nextStartTime.equals (nextItem.getStartTime())) {
          // Already set at right time -- do nothing
        } else {
          nextItem.setStartTime (nextStartTime);
          itemsModified++;
          // modify (nextItem);
        }
        duration = nextItem.getDurationAsInt();
        if (duration > 0) {
          nextStartTime.add (nextItem.getDuration());
        }
      } // end if not deleted and still open
    } // end for loop
  ...
  public String getColumnName (int column) {
    ToDoColumn tdcolumn = (ToDoColumn) displayColumns.get (column);
  ...
  /**
    ...
    @param  index The index pointing to an item in the underlying ToDoItems collection.
   */
  ...
  /**
    ...
    @return The ToDoItem pointed to by the passed index, or null
  ...
  public ToDoItem get (int index) {
    if (index < 0 || index >= size()) {
      ...
      return (ToDoItem)items.get(proxy.getIndex());
    }
  }
  ...
  /**
    ...
    @return The underlying index pointing to a ToDoItem, or -1
            if the index is out of range.
